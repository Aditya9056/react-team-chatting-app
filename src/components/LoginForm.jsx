...
import { CHAT_ENGINE_PROJECT_ID, DEMO_USERS } from '../config/chatEngine';
...
const LoginForm = () => {
	...
	useEffect(() => {
		DEMO_USERS.forEach((user, index) => {
			toast(`User${index + 1}: ${user.username} & Password: ${user.password}`);
		});
	}, []);
		...
		<div className='wrapper'>
			<div className='form'>
				...
				<form onSubmit={handleSubmit}>
					...
					<h2 className='error' style={{ color: 'orange' }}>
						{error}
					</h2>
				</form>
			</div>
